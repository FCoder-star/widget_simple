...
- **技术要点**：
  - **核心算法**：阿基米德螺旋布局算法，从中心点沿螺旋线向外扫描寻找词语放置位置；碰撞检测通过矩形重叠判断避免词语覆盖；支持7种形状的精确边界判断
  - **数学实现**：心形参数方程、五边形射线法判断、三角形叉积法、菱形曼哈顿距离归一化
  - **渲染优化**：Path缓存机制减少重复生成；按权重降序放置确保重要词语优先显示；四角检测防止词语溢出形状边界
  - **视觉设计**：HSV颜色空间生成4种配色方案；圆形/心形使用径向渐变，其他形状使用线性渐变；权重映射到10-38px字体范围；可选形状轮廓显示
  - **交互功能**：ChangeNotifier状态管理；随机种子支持重新生成布局；RepaintBoundary实现3倍像素率PNG导出
  - **性能考虑**：螺旋扫描最多2000次尝试，超出1.5倍半径提前退出；CustomPainter的shouldRepaint精确控制重绘时机
